...
async def fetch_info(chat, event):
    ...
    caption += f"ID: <i>{chat_obj_info.id}</i>\n"
    ...
    if created is not None:
        caption += f"Created: <i>{created.date().strftime('%b %d, %Y')} - {created.time()}</i>\n"
    else:
        caption += f"Created: <i>{chat_obj_info.date.date().strftime('%b %d, %Y')} - {chat_obj_info.date.time()}</i> {warn_emoji}\n"
    caption += f"Data Centre ID: {dc_id}\n"
    if exp_count is not None:
        chat_level = int((1 + sqrt(1 + 7 * exp_count / 14)) / 2)
        caption += f"{chat_type} level: <i>{chat_level}</i>\n"
    if messages_viewable is not None:
        caption += f"Viewable messages: <i>{messages_viewable}</i>\n"
    if messages_sent:
        caption += f"Messages sent: <i>{messages_sent}</i>\n"
    elif messages_sent_alt:
        caption += f"Messages sent: <i>{messages_sent_alt}</i> {warn_emoji}\n"
    if members is not None:
        caption += f"Members: <i>{members}</i>\n"
    if admins is not None:
        caption += f"Administrators: <i>{admins}</i>\n"
    if bots_list:
        caption += f"Bots: <i>{bots}</i>\n"
    if members_online:
        caption += f"Currently online: <i>{members_online}</i>\n"
    if restrcited_users is not None:
        caption += f"Restricted users: <i>{restrcited_users}</i>\n"
    if banned_users is not None:
        caption += f"Banned users: <i>{banned_users}</i>\n"
