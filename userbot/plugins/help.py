...
async def cmd_list(event):
    ...
    if input_str == "text":
        string = (
            "Total {count} commands found in {plugincount} plugins of GoodCatX\n\n"
        )
        catcount = 0
        plugincount = 0
        ...
        if len(string) > 4095:
            ...
            reply_text = f"**All commands of the GoodCatX can be seen [here]({url})**"
    ...
    if input_str:
        ...
        if HELPTYPE is True:
            ...
            string = "<b>Please specify which plugin do you want help for !!\
                \nNumber of plugins : </b><i>{count}</i>\
                \n<b>Usage:</b> <i>.help plugin name</i> \n\n"
            catcount = 0
            for i in sorted(CMD_LIST):
                string += "◆ " + f"<i>{str(i)}</i>"
                string += " "
...
async def info(event):
    ...
    input_str = event.pattern_match.group(1)
    if input_str == "text":
        string = "Total {count} commands found in {plugincount} sudo plugins of GoodCatX\n\n"
        catcount = 0
        plugincount = 0
        ...
        if len(string) > 4095:
            ...
            url = f"https://nekobin.com/{key}"
            reply_text = f"All commands of the GoodCatX are [here]({url})"
...
async def info(event):
    ...
    if args:
        ...
        string = "<b>Please specify which plugin do you want help for !!\
            \nNumber of plugins : </b><i>{count}</i>\
            \n<b>Usage : </b><i>.info plugin name</i>\n\n"
        catcount = 0
