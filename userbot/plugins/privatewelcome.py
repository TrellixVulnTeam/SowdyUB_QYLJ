...
async def show_welcome(event):
    ...
    cws = getcurrent_welcome_settings(event.chat_id)
    if not cws:
        await edit_or_reply(event, "__No pwelcome message saved here.__")
        return
    if cws.f_mesg_id:
        msg_o = await bot.get_messages(entity=BOTLOG_CHATID, ids=int(cws.f_mesg_id))
        await edit_or_reply(
            event, "__I am currently pwelcoming new users with this welcome note.__"
        )
...
CMD_HELP.update(
    {
        "privatewelcome": "**Plugin :** __privatewelcome__\
\n\n  •  **Syntax :** __.savepwel__ <welcome message> or reply to a message with .savepwel\
