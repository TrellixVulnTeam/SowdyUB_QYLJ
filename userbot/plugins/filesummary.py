# file summary plugin for GoodCatX  by @mrconfused
...
async def _(event):
    ...
    try:
        ...
        return await edit_delete(
            event, f"<b>Error : </b><i>{str(e)}</i>", time=5, parse_mode="HTML"
        )
    ...
    catevent = await edit_or_reply(
        event,
        f"<i>Counting files and file size of </i><b>{link}</b>\n<i>This may take some time also depends on number of group messages</i>",
        parse_mode="HTML",
    )
    media_dict = {
    ...
    for mediax in TYPES:
        ...
        if media_dict[mediax]["count"] != 0:
            largest += f"  •  <b><a href='{media_dict[mediax]['max_file_link']}'>{mediax}</a>  : </b><i>{humanbytes(media_dict[mediax]['max_size'])}</i>\n"
    ...
    totalstring = f"<i><b>Total files : </b>       | {str(totalcount)}\
                  \nTotal file size :    | {humanbytes(totalsize)}\
                  \nAvg. file size :     | {avghubytes}\
                  \n</i>"
    runtimestring = f"<i>Runtime :            | {runtime}\
                    \nRuntime per file :   | {avgruntime}\
                    \n</i>"
    line = "<i>+--------------------+-----------+</i>\n"
    result = f"<b>Group : {link}</b>\n\n"
    result += f"<i>Total Messages: {msg_count}</i>\n"
...
async def _(event):
    ...
    try:
        ...
        return await edit_delete(
            event, f"<b>Error : </b><i>{str(e)}</i>", 5, parse_mode="HTML"
        )
    try:
    ...
    catevent = await edit_or_reply(
        event,
        f"<i>Counting files and file size by </i>{_format.htmlmentionuser(userdata.first_name,userdata.id)}<i> in Group </i><b>{link}</b>\n<i>This may take some time also depends on number of user messages</i>",
        parse_mode="HTML",
    )
